      ...
      {bmi > 0 && (
        <>
          ...
          <View style={styles.bmiScale}>
            <View
              style={[styles.bmiRange, bmi < 18.5 && styles.bmiRangeActive]}
            >
            ...
            <View
              style={[
                styles.bmiRange,
                bmi >= 18.5 && bmi < 25 && styles.bmiRangeActive,
              ]}
            >
            ...
            <View
              style={[
                styles.bmiRange,
                bmi >= 25 && bmi < 30 && styles.bmiRangeActive,
              ]}
            >
    ...
    <View style={[styles.infoItem, lastItem && styles.lastInfoItem]}>
      ...
      {isEditing ? (
        <View style={styles.inputWithUnit}>
          <TextInput
            style={[
              styles.infoItemInput,
              multiline && styles.multilineInput,
              unit && styles.inputWithUnitField,
            ]}
            value={value}
          ...
          <Text
            style={
              selectedOption
                ...
                : styles.optionPickerPlaceholder
            }
          >
      ...
      {isEditing ? (
        <>
          ...
          >
            <Text
              style={
                value ? styles.optionPickerText : styles.optionPickerPlaceholder
              }
            >
  ...
  ({
    ...
    const handleSave = useCallback(async () => {
      ...
      if (heightNum && heightNum < 50) {
        Alert.alert(
          "Invalid Height",
          "Please enter height in centimeters (minimum 50 cm)",
        );
        return;
      }
      ...
      if (weightNum && weightNum < 2) {
        Alert.alert(
          "Invalid Weight",
          "Please enter weight in kilograms (minimum 2 kg)",
        );
        return;
      }
      ...
      <View style={styles.section}>
        ...
        <View style={styles.sectionCard}>
          ...
          {isEditing && (
            <TouchableOpacity
              style={[
                styles.saveSectionButton,
                saving && styles.saveSectionButtonDisabled,
              ]}
              onPress={handleSave}
              disabled={saving}
            >
...
const ProfileScreen: React.FC = () => {
  const navigation =
    useNavigation<StackNavigationProp<RootStackParamList, "Profile">>();
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      // Fetch patient info
      const patientInfoResponse = await fetch(
        `${API_BASE_URL}/api/patient/patient-info`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
    ...
    async (imageUri: string) => {
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/api/patient/upload-avatar`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const updateEmergencyContact = useCallback(
    async (contactInfo: EmergencyContact) => {
      try {
        const token = await AsyncStorage.getItem("authToken");
        if (!token) {
        ...
        const response = await fetch(
          `${API_BASE_URL}/api/patient/emergency-contact`,
          {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.message || "Failed to update emergency contact",
          );
        }
  ...
  const updatePhysicalMetrics = useCallback(async (metrics: any) => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.message || "Failed to update physical metrics",
        );
      }
  ...
  const updateMedicalInformation = useCallback(async (medicalInfo: any) => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.message || "Failed to update medical information",
        );
      }
            ...
            {!isEditing ? (
              ...
              <View style={styles.editActions}>
                ...
                >
                  <LinearGradient
                    colors={
                      saving ? ["#CBD5E0", "#A0A0A0"] : ["#4A90E2", "#63A4FF"]
                    }
